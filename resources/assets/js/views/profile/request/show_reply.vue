        ...
        <div class="row">
            <div class="col-lg-7 col-md-7 col-xs-12">
                <div class="exchange_request_title">
                    <h3 class="exchange_number request_title">Ответ на завявку {{reply.exchange_number}}</h3>
                ...
                <div class="row payment_info light_green short_margin">
                    ...
                    <div class="col-lg-5 col-md-5 col-xs-5 select_wallet_row_box">
                        <p>{{wallet_from}}</p>
                    </div>
                </div>
            ...
            <div class="col-lg-5 col-md-5 col-xs-12 service_information panel-view_request">
                <div class="content">
                    <h4>Статус заявки</h4>
                    <ul class="rowed serv">
                        <li><p>Ваш платеж на кошелек сервиса: <strong>{{display_amount_to}}</strong></p></li>
                    ...
                    <ul class="rowed serv">
                        <li><p>Платеж от пользователя: <strong>{{currency_from}}</strong></p></li>
                    ...
                    <ul class="rowed serv">
                        ...
                        <li v-if="reply.status_id === 4" class="status-yellow"><p>не выполнен</p></li>

                        <li v-if="reply.status_id === 1" class="status-red"><p>Отклонен</p></li>

                        <li v-if="reply.status_id === 3" class="status-green"><p>Подтвержден</p></li>

                        <li v-if="reply.status_id === 2" class="status-yellow"><p>не выполнен</p></li>
...
<script>
    ...
    export default {
        ...
        computed: {
            ...
            currentRate(){
                return parseFloat(this.reply.rate);
            },

            transaction(){
                return this.reply.transaction && this.reply.transaction.name || '1';
            },

            payment(){
                return this.reply.payment && this.reply.payment.name || '1';
            },
            ...
            amount_from(){
                if (!_.isEmpty(this.reply.amount_to)){
                    return parseFloat(this.reply.amount_to);
                }
            },

            amount_to(){
                if (!_.isEmpty(this.reply.amount_from)){
                    return parseFloat(this.reply.amount_from);
                }
            },

            amount_get(){
                if (this.amount_to > 0){
                    return parseFloat(this.reply.amount_get);
                }
            },

            commission(){
                if (this.amount_get > 0){
                    return parseFloat(this.reply.commission);
                }
            },
            ...
            getRequest(id){
                axios.get(`api/getRequest/${id}`)
                    .then((response) => {
                        ...
                        console.log(this.request, this.reply, response.data)
                    });
            }
        },
